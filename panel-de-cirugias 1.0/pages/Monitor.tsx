...
// In-memory cache for instant monitor opening
let monitorCache: {
    roomData: RoomMonitorData[];
    timestamp: number;
} | null = null;
...
const Monitor: React.FC = () => {
    ...
    const [minimizedForms, setMinimizedForms] = useState<MonitorCase[]>([]);
    const [activeForms, setActiveForms] = useState<MonitorCase[]>([]);
    const [manuallyExpandedRooms, setManuallyExpandedRooms] = useState<string[]>([]);

    const toggleRoomExpansion = (roomId: string) => {
        setManuallyExpandedRooms(prev => 
    ...
    const fetchMonitorData = async (forceRefresh = false) => {
        if (!forceRefresh && monitorCache) {
            setRoomsData(monitorCache.roomData);
            setLoading(false);
            if (Date.now() - monitorCache.timestamp < 15000) {
                return;
            }
        }
        ...
        try {
            ...
            // Parallel fetch of operating rooms, today's surgeries, and reschedule alerts
            const [roomsRes, sxRes, alertsRes] = await Promise.all([
                supabase
            ...
            const sanitizedSurgeries = surgeriesWithAlerts.map(s => {
                ...
                return {
                    ...
                    suspension_requested: s.suspension_requested || false,
                    reschedule_requested: s.reschedule_requested || false
                };
            });
    ...
    const formatTime = (date: Date) => {
        return date.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false, timeZone: 'America/Argentina/Buenos_Aires' });
    };

    const formatDate = (date: Date) => {
        return date.toLocaleDateString('es-ES', { weekday: 'long', day: 'numeric', month: 'long', timeZone: 'America/Argentina/Buenos_Aires' });
    };
    ...
    const handleStartDelay = async (surgery: MonitorCase, room: OperatingRoom) => {
        ...
        try {
            ...
            const updates = (subsequent || []).map(s => {
                ...
                const newTime = `${pad(newH)}:${pad(newM)}:${pad(sec || 0)}`;
    ...
    const handleExtendSurgery = async (surgery: MonitorCase, room: OperatingRoom) => {
        ...
        try {
            ...
            const updates = (subsequent || []).map(s => {
                const [h, m, sec] = s.start_time.split(':').map(Number);
                const totalMinutes = h * 60 + m + extension;
                const newH = Math.floor(totalMinutes / 60) % 24;
                const newM = totalMinutes % 60;
                const pad = (n: number) => n.toString().padStart(2, '0');
                const newTime = `${pad(newH)}:${pad(newM)}:${pad(sec || 0)}`;
    ...
    const handleMinimizeForm = () => {
        if (!selectedSurgeryForForm) return;
        
        if (activeForms.length >= 3) {
            alert('Solo puedes tener hasta 3 fichas abiertas o minimizadas al mismo tiempo. Por favor, cierra una antes de abrir otra.');
            return;
        }

        // Add to active forms if not already there
        if (!activeForms.some(f => f.id === selectedSurgeryForForm.id)) {
            setActiveForms(prev => [...prev, selectedSurgeryForForm]);
        }

        // Check if already minimized
        if (minimizedForms.some(f => f.id === selectedSurgeryForForm.id)) {
            setShowFormModal(false);
            setSelectedSurgeryForForm(null);
            return;
        }

        setMinimizedForms(prev => [...prev, selectedSurgeryForForm]);
        setSelectedSurgeryForForm(null);
        setShowFormModal(false);
    };

    const handleOpenForm = (surgery: MonitorCase) => {
        if (!activeForms.some(f => f.id === surgery.id)) {
            if (activeForms.length >= 3) {
                alert('Solo puedes tener hasta 3 fichas abiertas. Cierra una para abrir esta.');
                return;
            }
            setActiveForms(prev => [...prev, surgery]);
        }
        
        // Ensure it's not in minimized if we are opening it
        setMinimizedForms(prev => prev.filter(f => f.id !== surgery.id));
        setSelectedSurgeryForForm(surgery);
        setShowFormModal(true);
    };

    const handleCloseActiveForm = (id: string) => {
        setActiveForms(prev => prev.filter(f => f.id !== id));
        setMinimizedForms(prev => prev.filter(f => f.id !== id));
        if (selectedSurgeryForForm?.id === id) {
            setSelectedSurgeryForForm(null);
            setShowFormModal(false);
        }
    };

    const handleRestoreForm = (surgery: MonitorCase) => {
        setMinimizedForms(prev => prev.filter(f => f.id !== surgery.id));
        setSelectedSurgeryForForm(surgery);
        setShowFormModal(true);
    };

    const handleCloseMinimizedForm = (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
        setMinimizedForms(prev => prev.filter(f => f.id !== id));
        setActiveForms(prev => prev.filter(f => f.id !== id));
    };
    ...
    const CaseCard: React.FC<{ data: MonitorCase; room?: OperatingRoom; onOpenForm?: (sx: MonitorCase) => void }> = ({ data, room }) => {
        ...
        if (data.status === 'previous') {
            return (
                <div className={`bg-[#1F262E]/50 rounded-lg border-l-4 border-slate-500 opacity-60 grayscale group hover:opacity-100 hover:grayscale-0 transition-all ${isCompact ? 'p-2 mb-2' : 'p-4 mb-4'}`}>
                    ...
                    <h4 className={`font-bold text-slate-300 truncate flex items-center gap-2 uppercase ${isCompact ? 'text-sm' : 'text-lg'}`}>
                        {data.patient}
                ...
                <div className={`bg-[#1F262E] rounded-lg border-l-4 border-blue-500/30 border border-white/5 relative overflow-hidden group ${isCompact ? 'p-2 mt-2' : 'p-4 mt-4'}`}>
                    <div className={`flex justify-between items-center ${isCompact ? 'mb-0.5' : 'mb-1'}`}>
                        <div className="flex items-center gap-2">
                            ...
                            {data.suspensionRequested && (
                                <span className={`bg-red-500 text-white font-bold uppercase tracking-wider rounded flex items-center gap-1 animate-pulse ${isCompact ? 'px-1 py-0.5 text-[8px]' : 'px-2 py-0.5 text-[10px]'}`}>
                                    <span className="material-symbols-outlined text-[10px]">report_problem</span> {isCompact ? 'SUSP.' : 'Suspensión Solicitada'}
                                </span>
                            )}
                            {(data.rescheduleRequested || data.hasPendingReschedule) && (
                                <span className={`bg-amber-500 text-white font-bold uppercase tracking-wider rounded flex items-center gap-1 ${isCompact ? 'px-1 py-0.5 text-[8px]' : 'px-2 py-0.5 text-[10px]'}`}>
                                    <span className="material-symbols-outlined text-[10px]">event_repeat</span> {isCompact ? 'REPROG.' : 'Reprogramación Solicitada'}
                                </span>
                            )}
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <h4 className={`font-bold text-white truncate flex items-center gap-2 uppercase ${isCompact ? 'text-sm' : 'text-xl'}`}>
                        {data.patient}
            ...
            <div className={`bg-card-dark rounded-xl border-l-8 border-primary shadow-[0_10px_30px_-10px_rgba(0,0,0,0.5)] ring-1 ring-white/10 overflow-hidden relative group ${isCompact ? 'p-3' : 'p-4 md:p-6'}`}>
                ...
                <div className={`flex flex-col relative z-10 ${isCompact ? 'gap-2' : 'gap-3 md:gap-4'}`}>
                    <div className="flex justify-between items-start">
                        <div className="flex flex-col gap-2">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {data.suspensionRequested && (
                                    <div className={`inline-flex items-center gap-2 rounded bg-red-600 text-white font-bold uppercase tracking-wider shadow-lg shadow-red-900/20 w-fit animate-pulse ${isCompact ? 'px-2 py-0.5 text-[8px]' : 'px-2 py-0.5 md:px-3 md:py-1 text-[10px] md:text-sm'}`}>
                                        <span className={`material-symbols-outlined ${isCompact ? 'text-[10px]' : 'text-xs md:text-sm'}`}>report_problem</span> Suspensión Solicitada
                                    </div>
                                )}
                                {(data.rescheduleRequested || data.hasPendingReschedule) && (
                            ...
                            {isTecnico && (
                                <div className="flex flex-wrap gap-2">
                                    <button
                                        ...
                                        className={`bg-red-600 hover:bg-red-700 text-white rounded font-black uppercase shadow-lg shadow-red-900/20 transition-all flex items-center gap-1 md:gap-2 ${isCompact ? 'px-2 py-1 text-[11px]' : 'px-2 py-0.5 md:px-3 md:py-1 text-[9px] md:text-xs'}`}
                                    >
                                        <span className={`material-symbols-outlined ${isCompact ? 'text-sm' : 'text-xs md:text-sm'}`}>check_circle</span> Finalizar
                                    </button>
                                    {room && (
                                        <button
                                            onClick={() => handleExtendSurgery(data, room)}
                                            className={`bg-amber-600 hover:bg-amber-700 text-white rounded font-black uppercase shadow-lg shadow-amber-900/20 transition-all flex items-center gap-1 md:gap-2 ${isCompact ? 'px-2 py-1 text-[11px]' : 'px-2 py-0.5 md:px-3 md:py-1 text-[9px] md:text-xs'}`}
                                        >
                                            <span className={`material-symbols-outlined ${isCompact ? 'text-sm' : 'text-xs md:text-sm'}`}>history</span> {isCompact ? '+T' : 'Extender'}
                                        </button>
                                    )}
                                    <button
                                        onClick={() => handleResetSurgery(data.id)}
                                        className={`bg-slate-600 hover:bg-slate-500 text-white rounded font-black uppercase shadow-lg shadow-slate-900/20 transition-all flex items-center gap-1 md:gap-2 opacity-50 hover:opacity-100 ${isCompact ? 'px-2 py-1 text-[11px]' : 'px-2 py-0.5 md:px-3 md:py-1 text-[9px] md:text-xs'}`}
                                        title="Reiniciar Contador"
                                    >
                                        <span className={`material-symbols-outlined ${isCompact ? 'text-sm' : 'text-xs md:text-sm'}`}>restart_alt</span> {isCompact ? 'Reset' : 'Reiniciar'}
                                    </button>
                                    {/* Ficha de Cirugía Button */}
                                    {(user?.canFillForms || user?.role === 'SuperAdmin') && (
                                        <button
                                            onClick={() => handleOpenForm(data)}
                                            className={`bg-indigo-600 hover:bg-indigo-700 text-white rounded font-black uppercase shadow-lg shadow-indigo-900/20 transition-all flex items-center gap-1 md:gap-2 ${isCompact ? 'px-2 py-1 text-[11px]' : 'px-2 py-0.5 md:px-3 md:py-1 text-[9px] md:text-xs'}`}
                                        >
                                            <span className={`material-symbols-outlined ${isCompact ? 'text-sm' : 'text-xs md:text-sm'}`}>description</span> {isCompact ? 'Ficha' : 'Ficha de Cirugía'}
                                        </button>
                                    )}
                                </div>
                            )}
                    ...
                    <div>
                        ...
                        <h3 className={`font-black text-white leading-tight truncate flex items-center gap-3 uppercase ${isCompact ? 'text-xl my-0.5' : 'text-2xl md:text-4xl my-0.5 md:my-1'}`}>
                            {data.patient}
    ...
    return (
        <div className="bg-dark-bg text-white h-full md:h-screen flex flex-col overflow-hidden font-display w-full relative md:absolute md:top-0 md:left-0 md:z-50 selection:bg-primary/30">
                ...
                {loading ? (
                    ...
                    <div className="flex-grow flex flex-col md:flex-row overflow-y-auto md:overflow-x-auto">
                        {roomsData.filter((_, idx) => {
                            ...
                            const isEmpty = !data.current && data.completed.length === 0 && data.next.length === 0;
                            const isCollapsed = isEmpty && !manuallyExpandedRooms.includes(data.room.id);

                            if (isCollapsed) {
                                return (
                                    <section
                                        key={data.room.id}
                                        onClick={() => toggleRoomExpansion(data.room.id)}
                                        className={`hidden md:flex flex-none w-[70px] flex-col items-center py-4 bg-[#11161b] hover:bg-[#171e25] border-r border-[#283039] cursor-pointer transition-all duration-300 relative group h-full select-none`}
                                        title={`Haga clic para expandir ${data.room.name}`}
                                    >
                                        {/* Number Badge */}
                                        <div className="flex items-center justify-center rounded-lg font-black text-xs border bg-slate-800/50 text-slate-400 border-slate-700 size-8 mb-4">
                                            {(originalIdx + 1).toString().padStart(2, '0')}
                                        </div>

                                        {/* Vertical Title */}
                                        <div className="flex-1 flex items-center justify-center overflow-hidden">
                                            <h2 className="font-bold text-slate-400 uppercase tracking-wider text-xs whitespace-nowrap [writing-mode:vertical-lr] rotate-180 group-hover:text-white transition-colors duration-200">
                                                {data.room.name}
                                            </h2>
                                        </div>

                                        {/* Indicator / Icon */}
                                        <div className="mt-4 flex flex-col items-center gap-2 text-slate-500 group-hover:text-slate-300 transition-colors">
                            ...
                            return (
                                <section
                                    key={data.room.id}
                                    className={`flex-none w-full md:flex-1 md:min-w-[320px] flex flex-col md:h-full relative ${originalIdx < roomsData.length - 1 ? 'border-b md:border-b-0 md:border-r border-[#283039]' : ''} ${originalIdx % 2 !== 0 ? 'bg-[#13181e]' : ''}`}
                                >
                                    <div className={`bg-surface-dark border-b border-[#283039] flex items-center justify-between sticky top-0 z-10 shadow-lg ${viewMode === 'compact' ? 'p-2 md:p-3' : 'p-3 md:p-4'}`}>
                                        ...
                                        {/* Collapse button for empty rooms that are manually expanded */}
                                        {isEmpty && (
                                            <button
                                                onClick={() => toggleRoomExpansion(data.room.id)}
                                                className="hidden md:flex items-center justify-center size-8 rounded-lg hover:bg-white/10 text-slate-400 hover:text-white transition-colors"
                                                title="Colapsar Quirófano"
                                            >
                                    ...
                                    <div className={`flex-1 overflow-y-auto relative bg-transparent scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent ${viewMode === 'compact' ? 'p-2' : 'p-4 md:p-6'}`}>
                                        <div className={viewMode === 'compact' ? 'flex flex-col gap-2' : 'flex flex-col gap-4'}>
                                            {data.completed.map(sx => (
                                                <CaseCard key={sx.id} data={sx} onOpenForm={handleOpenForm} />
                                            ))}

                                            <div>
                                                {data.current ? (
                                                    <CaseCard data={data.current} room={data.room} onOpenForm={handleOpenForm} />
                                                ) : (
                                            ...
                                            {data.next.map(sx => (
                                                <CaseCard key={sx.id} data={sx} room={data.room} onOpenForm={handleOpenForm} />
                                            ))}
                                        </div>
                                    </div>
                                </section>
            ...
            {/* Active and Minimized Forms (Keep mounted to preserve state) */}
            {activeForms.map((sx) => (
                <div 
                    key={sx.id} 
                    className={selectedSurgeryForForm?.id === sx.id ? 'contents' : 'hidden'}
                >
                    <Suspense fallback={
                        ...
                        <SurgeryForm
                            surgery={sx}
                            onClose={() => handleCloseActiveForm(sx.id)}
            ...
            {/* Minimized Forms Bar */}
            {minimizedForms.length > 0 && (
                <div className="fixed bottom-4 right-4 z-[60] flex flex-col gap-2 pointer-events-none">
                    {minimizedForms.map((sx) => (
                        <div 
                            key={`min-${sx.id}`}
                            onClick={() => handleRestoreForm(sx)}
                            className="pointer-events-auto bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-3 rounded-xl shadow-2xl border border-white/20 flex items-center gap-3 cursor-pointer animate-in slide-in-from-right-10 duration-300 group min-w-[280px]"
                        >
                            <div className="bg-white/20 rounded-lg p-1.5 flex items-center justify-center">
                                <span className="material-symbols-outlined text-xl">description</span>
                            </div>
                            <div className="flex-1 overflow-hidden">
                                <p className="text-[10px] font-black uppercase tracking-wider opacity-70 leading-none mb-1">Ficha Técnica</p>
                                <p className="text-sm font-bold truncate leading-tight">{sx.patient}</p>
                                <p className="text-[10px] opacity-80 truncate">{sx.procedure}</p>
                            </div>
                            <button 
                                onClick={(e) => handleCloseMinimizedForm(e, sx.id)}
                                className="size-8 rounded-full hover:bg-black/20 flex items-center justify-center transition-colors"
                            >
