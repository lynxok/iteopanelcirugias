...
let billingPlanillaCache: PlanillaRow[] | null = null;
let billingAdmissionsCache: { pendientes: BillingAdmission[]; facturados: BillingAdmission[] } | null = null;

const Billing = () => {
    const [loading, setLoading] = useState(!billingPlanillaCache);
    ...
    // ── Planilla state ──────────────────────────────────────────────
    const [planillaRows, setPlanillaRows] = useState<PlanillaRow[]>(billingPlanillaCache || []);
    const [planillaLoading, setPlanillaLoading] = useState(!billingPlanillaCache);
    ...
    // ── Fetch existentes ─────────────────────────────────────────────
    const fetchAdmissions = async (forceRefresh = false) => {
        const statusFilter = activeTab === 'pendientes' ? 'pendiente' : 'facturado';
        if (!forceRefresh && billingAdmissionsCache) {
            const cached = statusFilter === 'pendiente' ? billingAdmissionsCache.pendientes : billingAdmissionsCache.facturados;
        ...
        try {
            ...
            if (!billingAdmissionsCache) {
                billingAdmissionsCache = { pendientes: [], facturados: [] };
            }
            if (statusFilter === 'pendiente') {
                billingAdmissionsCache.pendientes = transformed;
            } else {
                billingAdmissionsCache.facturados = transformed;
            }
        } catch (err) {
    ...
    // ── Fetch Planilla ───────────────────────────────────────────────
    const fetchPlanilla = async (forceRefresh = false) => {
        if (!forceRefresh && billingPlanillaCache && billingPlanillaCache.length > 0) {
            setPlanillaRows(billingPlanillaCache);
            setPlanillaLoading(false);
            return;
        }

        try {
            if (!billingPlanillaCache) {
                setPlanillaLoading(true);
            }

            // Ejecutar ambas consultas en paralelo con Promise.all
            const [admRes, surgRes] = await Promise.all([
                supabase
                    .from('hospital_admissions')
                    .select(`
                        id,
