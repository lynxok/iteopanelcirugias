...
*   **Auto-Finalización**:
    ...
    *   **Protección Anti-Cierre Prematuro por OSER**: La condición de trámite cerrado en el portal de OSER (`oser_status = 'CERRADA'`) no debe cambiar automáticamente el estado clínico de la cirugía a `completed` si la fecha quirúrgica es futura (`surgery_date > fecha_actual`), evitando que cirugías pendientes aparezcan realizadas en el calendario.
...
*   **Oficina ART y Tablero de Control ART (`isArtUser`)**: Los usuarios con rol `Oficina ART` o `ART` acceden al **Tablero de Control ART** ([AdminDashboard.tsx](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/pages/AdminDashboard.tsx)) y al Listado General/Calendario.
    *   **Filtro Estricto de Cobertura ART**: En todas las vistas principales ([SurgeryList.tsx](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/pages/SurgeryList.tsx), [Calendar.tsx](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/pages/Calendar.tsx), [Dashboard.tsx](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/pages/Dashboard.tsx)), las consultas del rol `Oficina ART` filtran exclusivamente por aquellas coberturas cuya columna `type` en `quirofano.coverages` sea `'ART'` (`.eq('type', 'ART')`).
    *   **Carga y Creación de Cirugías de ART**: El rol `Oficina ART` cuenta con permiso `edit` en `surgeries` para dar de alta solicitudes quirúrgicas y registrar pacientes. El selector de Prestador en [PatientSection.tsx](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/components/surgery-detail/PatientSection.tsx) le ofrece únicamente aseguradoras de la categoría "Aseguradoras (ART)", y la validación en [useSurgeryDetail.ts](file:///c:/Users/ignac/OneDrive/ITEO%20-%20Personal/Desarrollos/Coordinacion%20quirofano%20-%20capital%20-%20internaciones/panel-de-cirugias%201.0/components/surgery-detail/hooks/useSurgeryDetail.ts) impide guardar cualquier cirugía que no pertenezca a una ART (`coverage.type === 'ART'`).
    *   **Regla de Nomenclatura e Integridad**: Toda aseguradora de riesgos de trabajo dada de alta en el sistema (ej. `"EL NORTE SEGUROS"`, `"IAPSER ART"`, etc.) debe registrarse con `type = 'ART'` y sin caracteres invisibles/no estándar para garantizar su visibilidad en el panel de ART.
*   **Rol Híbrido "Administrativo ART"**:
    *   **Capacidades Híbridas**: Combina las facultades operativas del rol `Administrativo` (acceso y edición plena sobre todas las cirugías del sanatorio sin restricción de prestador, programación, ficha clínica y logística) con las atribuciones y visibilidad de `Oficina ART` (acceso directo al Tablero de Control ART / Administrativo con columnas especializadas de *Pedido de autorización ART*, *Paciente Notificado*, confirmación de turnos, y capacidad de suspender/cancelar intervenciones quirúrgicas).
...
*   **Turno Mañana y Extensión a Turno Tarde:** 
    ...
    *   **Extensión de Cirugía a Turno Tarde (Tolerancia Configurable y Tiempo Excedente - Vigencia desde Septiembre 2026)**: Si una cirugía iniciada en el turno mañana con fecha a partir del **1 de septiembre de 2026 (`date >= '2026-09-01'`)** continúa y su horario de fin (`actual_end_time`) supera por más del margen de tolerancia configurado (por defecto **15 minutos**) la hora de corte (`> 15:15 hs` de lunes a jueves, o `> 14:15 hs` los viernes), y no comenzó después de las 19:00 hs, **se computa al grupo de técnicos de turno tarde** (50% Técnico Fijo y 50% Técnico de Guardia con fichaje presente).
        - **Vigencia Temporal**: Esta regla y su asignación automática rigen exclusivamente a partir del 1 de septiembre de 2026, evitando modificar los períodos cerrados históricos anteriores (agosto 2026 hacia atrás).
        - **Tiempo computable para el Turno Tarde**: El tiempo de cirugía liquidable a los instrumentistas de turno tarde se computa **exclusivamente desde la hora de corte + tolerancia** (ej. desde las 15:15 hs o 14:15 hs) hasta el horario de fin efectivo de la cirugía (`actual_end_time`). No se les liquida el tiempo previo transcurrido durante la mañana.
        - **Tarifa fija de práctica**: La práctica (monto fijo por nomenclador) se mantiene computada al 50% para los técnicos de turno tarde.
...
### Estadísticas de Internación y Enfermería (`HospitalizationStats.tsx`)
*   **Separación de Conceptos (Internaciones Reales vs Rotaciones de Cama)**:
    - **Internaciones Reales**: Cantidad de **pacientes únicos** con ingreso hospitalario en el período analizado. Evita el sesgo de multiplicidad generado por reubicaciones o cambios de cama en el mapa de enfermería.
    - **Rotaciones de Cama / Movimientos**: Total bruto de movimientos y asignaciones registradas en `quirofano.hospital_admissions` (ej. si un paciente fue trasladado de cama o se corrigió su asignación durante la estadía). Permite auditar la carga de trabajo operativo de enfermería.
