...
*   `[2026-07-31]`: Lanzada la versión **v3.10.73**. Generada nueva compilación y publicado release en GitHub para solucionar problemas en el proceso de actualización e instalación de la v3.10.72.
    *   **Facturación (Prácticas Ambulatorias)**: Modificada la consulta en `Billing.tsx` sustituyendo `.eq('status', 'completed')` por `.or('status.eq.completed,is_guardia.eq.true')` para incorporar las prácticas ambulatorias en la planilla. Se agregaron las opciones de filtrado interactivo `Solo Ambulatorias / Guardia` y `Solo Internaciones / Quirófano`.
    *   **Rol Administrativo de Guardias**: Verificada y validada la lógica de permisos condicionales `isReadOnly` en `useSurgeryDetail.ts`, garantizando que puedan modificar cirugías creadas por su mismo rol (`created_by_role`).
    *   **Diagnóstico y Resolución de Errores de Publicación**:
        1. *Error GH001 (File size limit > 100MB)*: Al intentar hacer `git add .`, se intentó incluir la carpeta `release/` con los instaladores `.exe` de más de 160MB bloqueando el push. *Solución*: Se ejecutó `git reset` para desasociar los binarios pesados del historial de commits de Git y mantener únicamente el código fuente y las distribuciones ligeras del web app.
        2. *Error HTTP 401 (Bad Credentials) en `gh release create`*: La CLI usaba un token estático caducado en la variable de entorno `GITHUB_TOKEN` que solapaba la credencial activa del keyring. *Solución*: Se invocó la creación del release inyectando explícitamente el token activo recuperado de `gh auth status --show-token`, logrando subir `PanelCirugias_ITEO_Setup.exe`, `latest.yml` y `PanelCirugias_ITEO_Setup.exe.blockmap` exitosamente a GitHub Releases.
