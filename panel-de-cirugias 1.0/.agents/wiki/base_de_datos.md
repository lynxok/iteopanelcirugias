...
*   **Gestión de Técnicos (28/07/2026)**:
    ...
    *   Tabla `quirofano.tecnico_rates`: Almacena tarifas de horas, guardias, IP de la clínica y tarifas por práctica del nomenclador. Incluye columnas `updated_at`, `updated_by`, `updated_by_name` para trazabilidad y auditoría de ediciones, y la columna `effective_from TEXT DEFAULT '2025-01'` con índice único `tecnico_rates_type_code_period_idx` sobre `(rate_type, COALESCE(practice_code, ''), effective_from)` para soportar múltiples registros de tarifas por períodos de vigencia temporal.
