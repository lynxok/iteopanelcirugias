...
const ExcelImporter: React.FC<ExcelImporterProps> = ({ onComplete }) => {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Pre-fetch Doctors, ORs and Coverages for mapping
            ...
            const { data: coverages } = await supabase.from('coverages').select('name, vendor_id');
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                try {
                    ...
                    // 5. Insert Surgery
                    const coverageName = row['Obra Social'] || '';
                    const foundCov = coverages?.find(c => c.name === coverageName);
                    const vendorId = foundCov?.vendor_id || null;
