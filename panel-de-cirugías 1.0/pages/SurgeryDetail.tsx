...
export const SurgeryDetail: React.FC = () => {
    ...
    const [suggestedDate, setSuggestedDate] = useState<string | null>(null); // New state
    ...
    const fetchSurgeryDetails = async (surgeryId: string) => {
        ...
        try {
            ...
            if (data) {
                ...
                setSuggestedDate(data.suggested_date || null);
    ...
    const handleToggleInternacion = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setInternacionNotifiedAt(atWhen);
    ...
    const isReadOnly = (currentUserRole === 'Administrativo de Guardias' && !isGuardia && !isNew);
    ...
    const handleSurgerySave = async () => {
        ...
        try {
            ...
            const surgeryPayload: any = {
                ...
                internacion_notified: internacionNotified,
                internacion_notified_by: internacionNotifiedBy,
                internacion_notified_at: internacionNotifiedAt,
    ...
    const handleResolveRequest = async (type: 'suspension' | 'reschedule', action: 'accept' | 'reject') => {
        ...
        try {
            ...
            if (type === 'suspension') {
                ...
                if (action === 'accept') {
                    setRescheduleModal({ ...rescheduleModal, isOpen: true, newDate: suggestedDate || '', newTime: startTime });
                    setSaving(false);
                    return;
                }
            }
        ...
        try {
            ...
            const { error } = await supabase
                ...
                .update({
                    ...
                    reschedule_requested: false, // Clear flag
                    suggested_date: null // Clear field
                })
                .eq('id', id);
                            ...
                            <div className="p-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                                    ...
                                    <div>
                                        ...
                                        <input className="w-full rounded border border-slate-200 bg-slate-50 text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary text-sm px-3 py-2 disabled:bg-slate-100 disabled:text-slate-500" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Teléfono / Celular" disabled={isReadOnly || currentUserRole === 'Ortopedia'} />
                                    </div>
                                    <div className="md:col-span-2">
                                        <label className="block text-xs font-semibold text-slate-500 mb-1.5">Domicilio</label>
                                        <input className="w-full rounded border border-slate-200 bg-slate-50 text-slate-900 focus:border-primary focus:ring-1 focus:ring-primary text-sm px-3 py-2 disabled:bg-slate-100 disabled:text-slate-500" type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Calle, Altura, Piso..." disabled={isReadOnly || currentUserRole === 'Ortopedia'} />
                                    </div>
                                    <div>
                        ...
                        <section className="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
                            ...
                            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <label className={`flex-1 text-center py-2 border rounded text-sm font-medium cursor-pointer transition-colors ${surgerySide === 'left' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'border-slate-200 text-slate-600 hover:bg-slate-50'} ${(isReadOnly || currentUserRole === 'Ortopedia') ? 'opacity-50 pointer-events-none' : ''}`}>
                                            <input
                                                type="radio"
                                                name="side"
                                                className="hidden"
                                        ...
                                        <label className={`flex-1 text-center py-2 border rounded text-sm font-medium cursor-pointer transition-colors ${surgerySide === 'right' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'border-slate-200 text-slate-600 hover:bg-slate-50'} ${(isReadOnly || currentUserRole === 'Ortopedia') ? 'opacity-50 pointer-events-none' : ''}`}>
                                            <input
                                                type="radio"
                                                name="side"
                                                className="hidden"
                                        ...
                                        <label className={`flex-1 text-center py-2 border rounded text-sm font-medium cursor-pointer transition-colors ${surgerySide === 'bilateral' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'border-slate-200 text-slate-600 hover:bg-slate-50'} ${(isReadOnly || currentUserRole === 'Ortopedia') ? 'opacity-50 pointer-events-none' : ''}`}>
                                            <input
                                                type="radio"
                                                name="side"
                                                className="hidden"
