...
const ErrorLogViewer: React.FC = () => {
    ...
    const handleResolveError = async (errorId: string) => {
        const resolution = prompt('Ingrese una nota explicando la solución:');
        ...
        try {
            ...
            setErrors(errors.map(e =>
                e.id === errorId
                    ? { ...e, status: 'RESOLVED', resolution, resolved_at: new Date().toISOString(), resolved_by: user?.name || 'Admin' }
                    : e
            ));
        } catch (err) {
    ...
    const filteredErrors = errors.filter(error => {
        ...
        const matchesStatus = filterStatus === 'ALL' || (error.status || 'OPEN') === filterStatus;
                ...
                <div className="flex justify-between items-end">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                            ...
                            Gestión de Errores y Telemetría
                        </h1>
                        <p className="text-sm text-slate-500 mt-1">Monitoreo, diagnóstico y seguimiento de incidencias del sistema.</p>
                    </div>
                                    ...
                                    {err.status === 'RESOLVED' ? (
                                        <div className="mb-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                <span className="text-xs font-bold text-green-700 uppercase">Solucionado por {err.resolved_by}</span>
                                                <span className="text-[10px] text-green-600">
                                                    el {err.resolved_at ? new Date(err.resolved_at).toLocaleString() : ''}
                                                </span>
                                            </div>
                                            <p className="text-xs text-green-800 italic">"{err.resolution}"</p>
                                        </div>
                                    ) : (
                                        <div className="mb-3 flex justify-end">
                                            <button
                                                onClick={() => handleResolveError(err.id)}
                                                className="flex items-center gap-1 px-3 py-1.5 bg-slate-100 hover:bg-green-50 text-slate-600 hover:text-green-700 border border-slate-200 hover:border-green-200 rounded text-xs font-medium transition-colors"
                                            >
