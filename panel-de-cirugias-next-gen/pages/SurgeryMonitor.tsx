...
const SurgeryMonitor: React.FC = () => {
    ...
    const handleStatusChange = async (surgeryId: string, newStatus: string) => {
        ...
        if (newStatus === 'delayed') {
            setDelayModal({ isOpen: true, surgeryId, delayMinutes: 30 });
            return;
        }
        ...
        try {
            ...
            captureError(null, {
                context: 'SurgeryMonitor.handleStatusChange.audit',
                severity: 'WARNING',
                metadata: {
                    user_name: user?.name,
            ...
            // Original attempt (Legacy compatibility if table exists, but we use central one now)
            supabase.from('audit_logs').insert({
                user_name: user?.name || 'Sistema',
                user_role: user?.role,
                action: 'STATUS_CHANGE',
                resource: 'Cirugía Monitor',
                resource_id: surgeryId,
                description: `Estado cambiado a: ${newStatus}`,
                meta: { source: 'SurgeryMonitor' }
    ...
    const handleDelayConfirm = async () => {
        ...
        const surgery = surgeries.find(s => s.id === delayModal.surgeryId);
        if (!surgery) return;

        try {
            // 1. Update the delayed surgery status
            await supabase
                .from('surgeries')
                .update({ status: 'delayed' })
                .eq('id', surgery.id);

            // 2. Find subsequent surgeries in the same OR and shift their times
            const subsequentSurgeries = surgeries.filter(
                s => s.orId === surgery.orId && s.startTime > surgery.startTime && s.id !== surgery.id
            );

            const affectedDoctorIds: string[] = [];

            for (const subSurgery of subsequentSurgeries) {
                const [hours, minutes] = subSurgery.startTime.split(':').map(Number);
                const newMinutes = minutes + delayModal.delayMinutes;
                ...
                if (subSurgery.doctorId) {
                    affectedDoctorIds.push(subSurgery.doctorId);
                }
            }

            // 3. Create notifications for affected doctors
            for (const doctorId of [...new Set(affectedDoctorIds)]) {
                // Find the user_id linked to this doctor_id
                ...
                if (userData) {
                    await supabase.from('notifications').insert({
                        user_id: userData.id,
                        message: `⚠️ Demora en Quirófano: Su cirugía ha sido reprogramada (+${delayModal.delayMinutes} min) debido a una demora en el procedimiento anterior.`,
                        read: false
                    });
                }
            }
            ...
            supabase.from('audit_logs').insert({
                ...
                resource: 'Cirugía Monitor',
                resource_id: surgery.id,
                description: `Demora de ${delayModal.delayMinutes} min aplicada. ${subsequentSurgeries.length} cirugías reprogramadas.`,
                meta: { source: 'SurgeryMonitor', affectedCount: subsequentSurgeries.length }
            }).then(({ error: auditError }) => {
                if (auditError) console.warn('Silent Audit Error:', auditError);
            });

            setDelayModal({ isOpen: false, surgeryId: null, delayMinutes: 30 });
            fetchData();
        } catch (err) {
            console.error('Error applying delay:', err);
            alert('Error al aplicar la demora');
        }
    };
    ...
    return (
        <div className="flex-1 h-full overflow-y-auto bg-slate-100 p-6 font-sans">
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex items-center gap-3">
                                <input
                                    ...
                                    onChange={(e) => setDelayModal({ ...delayModal, delayMinutes: parseInt(e.target.value) || 30 })}
                                    className="w-24 text-center rounded-lg border-slate-300 text-2xl font-black text-slate-900 p-3 focus:ring-2 focus:ring-amber-400 outline-none"
                                />
                                <span className="text-lg font-bold text-slate-600">minutos</span>
                            </div>
                            <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                                <p className="text-xs text-amber-700 font-medium">
                                    <span className="font-black">Nota:</span> Los médicos de las cirugías afectadas recibirán una notificación automática con la nueva hora estimada.
                                </p>
                            </div>
                        </div>
                        <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                            <button
                                onClick={() => setDelayModal({ isOpen: false, surgeryId: null, delayMinutes: 30 })}
